...
namespace StardropTools
{
    ...
    public class AnimatorHandler : MonoBehaviour
    {
        ...
        [Space]
        [SerializeField] bool getOrCreasteAnimationEventListener = true;
        ...
        public readonly CustomEvent             OnAnimStart         = new CustomEvent();
        public readonly CustomEvent             OnAnimComplete      = new CustomEvent();
                                                                    
        public readonly CustomEvent<int>        OnAnimStartID       = new CustomEvent<int>();
        public readonly CustomEvent<int>        OnAnimCompleteID    = new CustomEvent<int>();
                                                                    
        public readonly CustomEvent<AnimatorHandlerState>  OnAnimStateStart    = new CustomEvent<AnimatorHandlerState>();
        public readonly CustomEvent<AnimatorHandlerState>  OnAnimStateComplete = new CustomEvent<AnimatorHandlerState>();

        public CustomEvent<int> OnAnimEventINT          => animEventListener.OnAnimEventINT;
        public CustomEvent<string> OnAnimEventSTRING    => animEventListener.OnAnimEventSTRING;
        ...
        public void SyncAnimatorWithAnimationID(AnimatorHandlerAnimationType transitionType, bool disableOnFinish = false)
        {
            if (animator.GetCurrentAnimatorStateInfo(0).shortNameHash != CurrentState.StateHash)
            {
                ...
                {
                    case AnimatorHandlerAnimationType.Play:
                        animator.Play(CurrentState.StateHash, CurrentState.Layer);
                        break;

                    case AnimatorHandlerAnimationType.CrossFade:
                        animator.CrossFade(CurrentState.StateHash, CurrentState.Layer);
                        break;

                    case AnimatorHandlerAnimationType.Trigger:
        ...
        /// </summary>
        public void PlayQueued(int animID, int nextAnimID, AnimatorHandlerAnimationType animationType)
        {
            switch (animationType)
            {
                case AnimatorHandlerAnimationType.Play:
                    if (PlayAnimation(animID))
                    {
                        ...
                        nextAnimationTimer.OnTimerComplete.AddListener(() => PlayAnimation(nextAnimID));
                    }
                    break;
                ...
                case AnimatorHandlerAnimationType.CrossFade:
                    if (CrossFadeAnimation(animID))
                    {
                        float time = animStates[nextAnimID].LengthInSeconds;
                        nextAnimationTimer = new Timer(time).Play();
                        nextAnimationTimer.OnTimerComplete.AddListener(() => CrossFadeAnimation(nextAnimID));
                    }
                    break;
                ...
                case AnimatorHandlerAnimationType.Trigger:
                    if (TriggerAnimation(animID))
                    {
                        float time = animStates[nextAnimID].LengthInSeconds;
                        nextAnimationTimer = new Timer(time).Play();
                        nextAnimationTimer.OnTimerComplete.AddListener(() => TriggerAnimation(nextAnimID));
                    }
                    break;
            }
        }
        ...
        protected void AnimationLifetime(float time, bool disableOnFinish)
        {
            ...
            animationLifetimeTimer.OnTimerComplete.AddListener(() => AnimationLifetimeComplete(disableOnFinish));
        }
        ...
        void GetAnimator()
        {
            ...
            if (getOrCreasteAnimationEventListener == true && animator != null && animEventListener == null)
            {
