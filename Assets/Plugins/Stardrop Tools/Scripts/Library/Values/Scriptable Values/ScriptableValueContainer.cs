    ...
    public class ScriptableValueContainer : ScriptableValue
    {
        ...
        public override void Save(string key)
        {
            ...
            switch (valueType)
            {
                ...
                case ValueType.Vector2:
                    PlayerPrefs.SetString(key, new Vector2Value(vector2Value).ToString());
                    break;

                case ValueType.Vector3:
                    PlayerPrefs.SetString(key, new Vector3Value(vector3Value).ToString());
                    break;

                case ValueType.Quaternion:
                    PlayerPrefs.SetString(key, new QuaternionValue(quaternionValue).ToString());
                    break;
            }
        ...
        public override void Load(string key)
        {
            ...
            switch (valueType)
            {
                ...
                case ValueType.Vector2:
                    vector2Value = StardropTools.Values.Vector2Value.FromJson(PlayerPrefs.GetString(key)).ToVector2();
                    break;

                case ValueType.Vector3:
                    vector3Value = StardropTools.Values.Vector3Value.FromJson(PlayerPrefs.GetString(key)).ToVector3();
                    break;

                case ValueType.Quaternion:
                    quaternionValue = StardropTools.Values.QuaternionValue.FromJson(PlayerPrefs.GetString(key)).ToQuaternion();
                    break;
            }
        }
