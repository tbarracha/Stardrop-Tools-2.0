...
namespace StardropTools
{
    public static class UIUtils
    {
        ...
        public static readonly AnchorMinMax StretchMid = new AnchorMinMax(new Vector2(0, 0.5f), new Vector2(1, 0.5f));
        ...
        public static void SetRectTransformAnchor(RectTransform rectTransform, UIAnchor anchor)
        {
            switch (anchor)
            {
                ...
                case UIAnchor.StretchMid:
                    SetRectAnchorMinMax(rectTransform, StretchMid);
                    break;
        ...
        public static UIAnchor GetUIAnchor(RectTransform rectTransform)
        {
            ...
            // Top
            if (minMax.CompareAnchors(AnchorTopLeft))
                return UIAnchor.TopLeft;
            if (minMax.CompareAnchors(AnchorTopCenter))
                return UIAnchor.TopCenter;
            if (minMax.CompareAnchors(AnchorTopRight))
                return UIAnchor.TopRight;

            // Center
            if (minMax.CompareAnchors(AnchorCenterLeft))
                return UIAnchor.CenterLeft;
            if (minMax.CompareAnchors(AnchorCenter))
                return UIAnchor.Center;
            if (minMax.CompareAnchors(AnchorCenterRight))
                return UIAnchor.CenterRight;

            // Bottom
            if (minMax.CompareAnchors(AnchorBottomLeft))
                return UIAnchor.BottomLeft;
            if (minMax.CompareAnchors(AnchorBottomCenter))
                return UIAnchor.BottomCenter;
            if (minMax.CompareAnchors(AnchorBottomRight))
                return UIAnchor.BottomRight;

            // Stretch
            if (minMax.CompareAnchors(StretchLeft))
                return UIAnchor.StretchLeft;
            if (minMax.CompareAnchors(StretchTop))
                return UIAnchor.StretchTop;
            if (minMax.CompareAnchors(StretchRight))
                return UIAnchor.StretchRight;
            if (minMax.CompareAnchors(StretchBottom))
                return UIAnchor.StretchBottom;
            if (minMax.CompareAnchors(StretchFull))
        ...
        public static Vector2 WorldToCanvasPosition(Vector3 worldPosition, RectTransform canvasRectTransform)
        {
